...
public final class BedtimeStorage {
    ...
    public static void mirror(Context context) {
        ...
        e.putBoolean("setup_complete",normal.getBoolean("setup_complete",false)); e.putBoolean("last_active",normal.getBoolean("last_active",false)); e.putBoolean("accounts_confirmed",normal.getBoolean("accounts_confirmed",false)); e.putString("backend",normal.getString("backend","")); e.putString("child",normal.getString("child","child-001")); e.putString("child_token",normal.getString("child_token","")); e.apply();
    }
    ...
    public static void setLastActive(Context context,boolean active){context.getSharedPreferences(PREFS,MODE_PRIVATE).edit().putBoolean("last_active",active).apply();prefs(context).edit().putBoolean("last_active",active).apply();}
}
