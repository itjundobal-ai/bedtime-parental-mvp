...
<script>
const child=document.getElementById('child'), key=document.getElementById('key'), statusEl=document.getElementById('status');
...
async function setBedtime(active){
 ...
 localStorage.setItem('childId',id); localStorage.setItem('parentKey',key.value);
 ...
 try{
  ...
  const r=await fetch('/api/children/'+encodeURIComponent(id)+'/bedtime',{method:'POST',headers,body:JSON.stringify({active})});
  const d=await r.json(); if(!r.ok) throw new Error(d.error||('HTTP '+r.status));
  statusEl.innerHTML=active?'<span class="active">🌙 Bedtime is ON</span>':'<span class="inactive">✓ Bedtime is OFF</span>';
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET') {
      ...
      return json({ childId, active: stored?.active === true, updatedAt: stored?.updatedAt || null });
    }
    ...
    if (request.method === 'POST') {
      ...
      const state = { active: body.active, updatedAt: new Date().toISOString() };
